...
const flipButton = document.querySelector('.plus-icon')
...
const buttonCopyMail = document.querySelector('.button-copy')
...
const copyEmailToClipboard = () => {
  ...
  const previousHTML = buttonCopyMail.innerHTML

  input.select()
  navigator.clipboard
    .writeText(input.value)
    .then(() => {
      buttonCopyMail.classList.add('copied')
      buttonCopyMail.textContent = `COPIADO!`
      setTimeout(() => {
        buttonCopyMail.innerHTML = previousHTML
        buttonCopyMail.classList.remove('copied')
      }, 2000)
    })
    .catch(err => {
      console.error(err)
...
buttonCopyMail.addEventListener('click', copyEmailToClipboard)
